...
// mod lru_cached_reader;

use crate::juno_state_reader::{ptr_to_felt, JunoStateReader};
// use crate::lru_cached_reader::{LruCachedReader};
...
pub extern "C" fn cairoVMExecute(
    ...
    for sn_api_txn in sn_api_txns {
        ...
        let res = match txn.unwrap() {
            Transaction::AccountTransaction(t) => t.execute(&mut state, &block_context, charge_fee, validate),
            Transaction::L1HandlerTransaction(t) => t.execute(&mut state, &block_context, charge_fee, validate)
        };
